...
- **`src/review/scanner.ts`** — Runs all scanner models in parallel via `Promise.all()`. Each scanner sees only the diff, never other scanners' output. SKIPPED only on an exact `NO_FINDINGS` or an intentionally empty completion (`finish_reason: stop`); empty-via-truncation is FAILED. `runScanners` returns `{ results, coverage }` and runs an automatic rescue pass for roles with zero successful scanners (`rescue-models` input, else fastest successful model). `runJudgeScan` makes the judge model's own isolated scan (result model `judge-scan:<model>`) — the aggregation judge stays a pure verifier and never sees its own findings as privileged.
- **`src/review/judge.ts`** — Aggregates scanner results. Summary mode: free-form text. Inline mode: instructs judge to output JSON `InlineFinding[]`, parses with `parseInlineFindings()` (strips markdown fences, validates fields). JSON parse failure → `findings: undefined` → triggers summary fallback. Refuses (`success: false`, no model call) on a pool without usable output — `index.ts` classifies the pool first (see `verdict.ts`).
- **`src/review/verdict.ts`** — v0.5.3 pool classification, run in `index.ts` between the `min-successful-scanners` gate and the judge. `classifyScannerPool()` → `findings` (judge runs; FAILED scanners are stamped on the verdict headline via `formatDegradedSuffix()` + `appendDegradedSuffix()`), `all-clear` (deterministic APPROVE text, judge not called), `incomplete` (deterministic INCOMPLETE text, action exits 1). `hasUsableOutput()` lives here; the judge prompt builders throw on a pool without usable output — there is no "review could not be completed" prompt any more. Verdict texts are tr/en (default tr).
